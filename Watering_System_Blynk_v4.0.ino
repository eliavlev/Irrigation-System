...
char auth[] = "76e1e5bf26954ff9a7dd649ad58543fd";
...
int timeZoneOffset = 2; // set this to the offset in hours to your local time;
...
// Read default watering time from FLASH
  ...
  StartTime_Valve1=ReadFromFlash("Start_v2");
  StopTime_Valve1=ReadFromFlash("Stop_v2");
  StartTime_Valve1=ReadFromFlash("Start_v3");
  StopTime_Valve1=ReadFromFlash("Stop_v3");
...
void WateringCheck(){
  ...
  WidgetLED led7(V7); // Valve 3 register to virtual pin 7
  ...
  // if valve 1=True & current weekday = Active watering days & current time within watering time interval
  if (WeekDays_Valve1[0]==1 && WeekDays_Valve1[weekday()]==1 && timeVal >=StartTime_Valve1 && timeVal<=StopTime_Valve1){
    Water(RELAY1,"Valve1");
  ...
  if (WeekDays_Valve2[0]==2 && WeekDays_Valve2[weekday()]==1 && timeVal >=StartTime_Valve2 && timeVal<=StopTime_Valve2){
  ...
  if (WeekDays_Valve3[0]==3 && WeekDays_Valve3[weekday()]==1 && timeVal >=StartTime_Valve3 && timeVal<=StopTime_Valve3){
...
void Water(int relay, String relayName){
     ...
     // Serial.println("Watering "+relayName);
      //String toDisplay= "Watering ";
      //String phrase1=(toDisplay+ relayName);
      //phrase1.toCharArray(phrase, 17);
      //PrintToLCD(phrase,0,30);
...
int ReadFromFlash2(String filename, uint8_t WeekArray[]){
  if (spiffsActive) {
   ...
   void SetTimeZone(){
      if ((month()*100+day())>=323 and (month()*100+day())<=1029){
          timeZoneOffset =3;
       }
      else{
        timeZoneOffset=2; 
